...
class JungleRunner {
  ...
  buildPlayer() {
    ...
    const shirtColor = this.equipped.outfit?.reward_id === "reward_jungle_cape" ? 0x8b2f3c : 0xf0b429;
    ...
    if (this.equipped.outfit?.reward_id === "reward_jungle_cape") {
      const cape = new T.Mesh(new T.PlaneGeometry(1.0,1.25), new T.MeshStandardMaterial({color:0x8b2f3c,side:T.DoubleSide,roughness:.8}));
      cape.position.set(0,1.7,-.5); cape.rotation.x=-.12; root.add(cape); this.playerParts.cape=cape;
    }
    if (this.equipped.weapon?.reward_id === "reward_wooden_sword") {
      const sword = new T.Group();
      const blade = new T.Mesh(new T.BoxGeometry(.12,1.0,.12),this.mat(0x9b6b3f,.8)); blade.position.y=.5; sword.add(blade);
      const guard = new T.Mesh(new T.BoxGeometry(.48,.1,.12),this.mat(0x5d3b22,.85)); sword.add(guard);
      sword.position.set(.72,1.25,-.12); sword.rotation.z=-.35; root.add(sword); this.playerParts.sword=sword;
    }
    if (this.equipped.pet?.reward_id === "reward_pet_parrot") this.buildParrot();
  ...
  collect(e) {
    ...
    else if(e.type==="key"){this.keys++;this.score+=180;this.combo+=2;this.maxCombo=Math.max(this.maxCombo,this.combo);this.audio.key();this.showFeedback("🔑","Golden Key!",`${this.keys}/${this.config.keyTarget} mission keys collected.`,true,650);}
  ...
  showWinResult(result) {
    ...
    const nextButton=result.newlyUnlockedLevelId?`<a class="runner-big-btn" href="jungle-game.html?level=${encodeURIComponent(result.newlyUnlockedLevelId)}" style="display:inline-flex;align-items:center;text-decoration:none;">Play Next Level →</a>`:"";
    ui.resultModal.innerHTML=`
      ...
      <p class="sub">Great run! You survived the jungle and passed the learning gates.</p>
